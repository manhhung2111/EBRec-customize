...
from models.EBRec import EBRec
...
def get_cmd():
    ...
    parser.add_argument("-m", "--model", default="EBRec", type=str, help="which model to use, options: EBRec")
...
def main():
    ...
    assert paras["model"] in ["EBRec"], "Pls select models from: EBRec"
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        # model
        if conf['model'] == 'EBRec':
            model = EBRec(conf, dataset.graphs).to(device)
        else:
...
def write_log(run, log_path, topk, step, metrics, epoch):
    ...
    val_str = "%s, epoch_%d, Top_%d, Val:  recall: %f, ndcg: %f" %(curr_time,epoch, topk, val_scores["recall"][topk], val_scores["ndcg"][topk])
    test_str = "%s, epoch_%d, Top_%d, Test: recall: %f, ndcg: %f" %(curr_time,epoch, topk, test_scores["recall"][topk], test_scores["ndcg"][topk])

    log = open(log_path, "a")
    log.write("%s\n" %(val_str))
    log.write("%s" %(test_str))
    log.close()
